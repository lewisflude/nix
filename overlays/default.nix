{
  ...
  mkFlakePackage = inputName: packageName: fallback: _final: prev: let
  ...
  overlaySet = {
    ...
    nix-topology = mkOptionalOverlay (inputs ? nix-topology && inputs.nix-topology ? overlays) inputs.nix-topology.overlays.default;
    ...
    niri = mkOptionalOverlay (isLinux && inputs ? niri && inputs.niri ? overlays) inputs.niri.overlays.niri;
