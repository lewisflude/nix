#!/usr/bin/env bash
#
# organize-untracked-music-production.sh
#
# Organize music production files that aren't tracked by torrent clients
# (completed torrents, manual downloads, etc.)
#
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Music Production Manual File Organization                ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [[ "$DRY_RUN" == false ]]; then
  for category in "${!categories[@]}"; do
    ssh "${HOST}" "mkdir -p '${MUSIC_PROD_BASE}/${category}'" 2>/dev/null || true
  done
...
for file in "${!FILE_MAP[@]}"; do
  ...
  if [[ "$DRY_RUN" == false ]]; then
    ...
    if ! ssh "${HOST}" "test -e '${source_path}'" 2>/dev/null; then
    ...
    if ssh "${HOST}" "test -e '${dest_path}'" 2>/dev/null; then
    ...
    # Move the file
    if ssh "${HOST}" "mv '${source_path}' '${dest_path}'" 2>/dev/null; then
      log_success "  ✓ Moved"
