...
get_memory_info() {
  if is_macos; then
    ...
    local total_mem=$((($pages_free + $pages_active + $pages_inactive + $pages_wired) * $page_size / 1024 / 1024 / 1024))
    local used_mem=$((($pages_active + $pages_wired) * $page_size / 1024 / 1024 / 1024))
    local usage_percent=$((used_mem * 100 / total_mem))
    echo "${used_mem}GB/${total_mem}GB (${usage_percent}%)"
  else
...
system_overview() {
  ...
  echo -e "${CYAN}Load Average:${NC} $(get_load)"
  echo -e "${CYAN}Memory:${NC} $(get_memory_info)"
...
process_monitor() {
  ...
  local nix_procs=$(pgrep -af nix 2>/dev/null || true)
  if [[ -n "$nix_procs" ]]; then
  ...
  if is_macos; then
    ps aux -r | head -6 | tail -5 | while read -r line; do
...
optimization_suggestions() {
  ...
  # Check memory usage (skip on macOS as calculation is complex)
  if ! is_macos; then
