# AI Tools Aspect
#
# Combines all AI/ML-related configuration in a single file.
# Reads options from config.host.features.aiTools (defined in modules/shared/host-options/features/ai-tools.nix)
#
# Platform support:
# - NixOS: Ollama, Open WebUI, model pre-downloading
# - Darwin: Placeholder (Ollama works natively on macOS)
{
  config,
  lib,
  pkgs,
...
in
{
  ...
  config = mkMerge [
    ...
    (mkIf (cfg.enable && isDarwin) {
      # Ollama on macOS is typically installed via Homebrew or official installer
      # for native Metal acceleration support
      # This placeholder allows for future nix-darwin integration
    })
    ...
    # ================================================================
    {
      assertions = [
        {
          assertion = cfg.ollama.enable -> cfg.enable;
          message = "Ollama requires aiTools feature to be enabled";
        }
        {
          assertion = cfg.openWebui.enable -> cfg.enable;
          message = "Open WebUI requires aiTools feature to be enabled";
        }
        {
          assertion = !(cfg.ollama.acceleration == "cuda" && isDarwin);
          message = "CUDA acceleration is not available on macOS";
        }
        {
          assertion =
            isLinux -> ((cfg.ollama.acceleration == "cuda") -> config.hardware.graphics.enable);
          message = "Ollama CUDA acceleration requires hardware.graphics.enable to be true";
        }
      ];
    }
  ];
