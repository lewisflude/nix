...
let
  ...
  # Bash uses terminal ANSI colors that are themed by Signal
  colors = {
    text-dim = semantic.text "tertiary" themeMode;
    text-secondary = semantic.core "foreground" themeMode;
  };
...
in
{
  config = mkIf (cfg.enable && shouldTheme) {
    programs.bash.initExtra = ''
      ...
      *.jpg=${toAnsiRgb categorical.${themeMode}."data-viz-06"}:\
      *.jpeg=${toAnsiRgb categorical.${themeMode}."data-viz-06"}:\
      *.png=${toAnsiRgb categorical.${themeMode}."data-viz-06"}:\
      *.gif=${toAnsiRgb categorical.${themeMode}."data-viz-06"}:\
      *.mp3=${toAnsiRgb categorical.${themeMode}."data-viz-08"}:\
      *.mp4=${toAnsiRgb categorical.${themeMode}."data-viz-08"}:\
      *.avi=${toAnsiRgb categorical.${themeMode}."data-viz-08"}:\
      *.mkv=${toAnsiRgb categorical.${themeMode}."data-viz-08"}"
    ...
    programs.bash.bashrcExtra = lib.mkIf (!config.programs.bash ? promptInit) ''
      ...
      if [ -z "$PROMPT_COMMAND" ]; then
        PS1="${toPs1Rgb accent.primary.Lc75}\\u${toPs1Rgb colors.text-secondary}@${toPs1Rgb accent.secondary.Lc75}\\h ${toPs1Rgb accent.secondary.Lc75}\\w ${toPs1Rgb colors.text-secondary}\\$ \\[\\e[0m\\]"
      fi
    '';
  };
}
