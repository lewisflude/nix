    ...
    lib.mkIf pkgs.stdenv.isDarwin {
      ...
      home.file.".config/karabiner/assets/complex_modifications/mnk88-wkl.json" = {
        text = builtins.toJSON {
          ...
          rules = [
            { description = "MNK88: Caps Lock → Control"; manipulators = [ (keyToKey "caps_lock" "left_control") ]; }
            { description = "MNK88: Swap Option↔Command, Control↔Option"; manipulators = [
            ...
            { description = "MNK88: Launcher keys (F13 → Ghostty, Print Screen → Browser)"; manipulators = [
              (keyToShell "f13" "open -a Ghostty")
              (keyToShell "print_screen" "open -a \"Safari\" || open -a \"Arc\" || open -a \"Google Chrome\"")
            ]; }
            { description = "MNK88: Window management via F16–F19 (Rectangle chords)"; manipulators = [
