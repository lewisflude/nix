...
{ config, ... }:
{
  ...
  # ═══════════════════════════════════════════════════════════════════
  flake.modules.homeManager.gpg = { lib, pkgs, config, ... }:
    let
      isDarwin = pkgs.stdenv.isDarwin;
      gtkThemeName = config.theming.signal.gtk.themeName or null;
      pinCacheTtl = { gpg = 3600; ssh = 3600; };
    in
    {
      home.packages = [ pkgs.yubikey-manager ]
        ++ lib.optionals isDarwin [ pkgs.pinentry_mac pkgs.terminal-notifier ];

      programs.gpg = {
        enable = true;
        scdaemonSettings = { disable-ccid = true; }
          // lib.optionalAttrs isDarwin { pcsc-shared = true; disable-application = "piv"; };
        settings = {
          keyid-format = "0xlong";
      ...
      services.gpg-agent = {
        ...
        pinentry.package = if isDarwin then pkgs.pinentry_mac
          else pkgs.writeShellScriptBin "pinentry-auto" ''
