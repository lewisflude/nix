...
let
  ...
  darwinConfig = {
    ...
    launchd.daemons.nix-garbage-collection = {
      serviceConfig = {
        ProgramArguments = [
    ...
    launchd.daemons.nix-store-optimization = {
      serviceConfig = {
        ProgramArguments = [
  ...
  linuxConfig = {
    ...
    # Systemd timer for store optimization
    systemd.timers.nix-store-optimization = {
    ...
    systemd.services.nix-store-optimization = {
      serviceConfig = {
        Type = "oneshot";
...
lib.mkMerge [
  commonConfig
  (lib.mkIf (lib.hasInfix "darwin" builtins.currentSystem) darwinConfig)
  (lib.mkIf (lib.hasInfix "linux" builtins.currentSystem) linuxConfig)
]