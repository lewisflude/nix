...
in
{
  configurations.nixos.jupiter.module = { pkgs, ... }: {
    ...
    boot.kernelModules = [ "kvm-intel" "zfs" "hid_sony" ];
    ...
    fileSystems = {
      "/" = { device = "npool/root"; fsType = "zfs"; };
      "/home" = { device = "npool/home"; fsType = "zfs"; };
      "/boot" = { device = "/dev/disk/by-label/BOOT"; fsType = "vfat"; };
      "/var/lib/docker" = { device = "npool/docker"; fsType = "zfs"; };
      "/mnt/disk1" = {
      ...
      "/mnt/storage" = {
        ...
        options = [
          ...
          "category.create=mfs" "minfreespace=1G" "fsname=mergerfs"
    ...
    networking.firewall.allowedTCPPorts = [ 22 constants.ports.mcp.docs ];
    ...
    users.users.${username} = {
      ...
      extraGroups = [ "dialout" "admin" "wheel" "staff" "_developer" "git" "media" "audio" "uinput" "video" ];
    };

    # Packages
    environment.systemPackages = [ pkgs.mergerfs pkgs.xfsprogs ];
