{
  ...
  inherit (lib) mkIf mkEnableOption mkOption types optionalAttrs mapAttrs mapAttrs' nameValuePair recursiveUpdate;
...
in {
  ...
  config = mkIf (cfg.enable && cfg.janitorr.enable) (let
  ...
  in {
    ...
    virtualisation.oci-containers.containers.janitorr = {
      ...
      extraOptions =
        ["--network=host"] # Use host network to access native services
        ++ mkHealthFlags {
