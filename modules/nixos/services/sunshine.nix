    ...
    text = ''
      ...
      if run_as_user pgrep -x steam >/dev/null 2>&1; then
        ...
        if [ $LAUNCH_BIG_PICTURE -eq 1 ]; then
          ...
          run_as_user ${pkgs.steam}/bin/steam steam://open/bigpicture >/dev/null 2>&1 &
        ...
        if [ $LAUNCH_BIG_PICTURE -eq 1 ]; then
          ...
          run_as_user ${pkgs.steam}/bin/steam -gamepadui "$@" >/dev/null 2>&1 &
        else
          log "Launching Steam normally"
          run_as_user ${pkgs.steam}/bin/steam "$@" >/dev/null 2>&1 &
        fi
      fi
