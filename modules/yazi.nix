  ...
  flake.modules.homeManager.yazi = { lib, pkgs, config, ... }: {
    programs.yazi = {
      ...
      settings = {
        ...
        preview = { max_width = 1000; max_height = 1000; };
      };
      keymap = {
        mgr.prepend_keymap = [
          { on = "<C-n>"; run = ''shell -- dragon-drop -x -i -T "$1"''; desc = "Drag and drop files with dragon"; }
          { on = "<C-f>"; run = ''shell -- ya emit cd "$(find . -type d | fzf --preview 'ls -la {}')"''; desc = "FZF directory navigation"; }
