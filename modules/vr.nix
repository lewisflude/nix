# VR Feature Module - Dendritic Pattern
# Single file containing both NixOS system config and home-manager user config
# Usage: Import flake.modules.nixos.vr in host definition
{ config, ... }:
let
...
in
{
  ...
  # ==========================================================================
  flake.modules.nixos.vr = { pkgs, lib, config, ... }:
  let
    inherit (lib) mkIf mkMerge mkDefault makeLibraryPath;
  in
  {
    # ========================================================================
    # WiVRn Configuration
    # ========================================================================
    # Wireless VR streaming configuration with embedded Monado runtime
    # Provides wireless PCVR from Quest headsets over WiFi
    services.wivrn = {
      enable = true;
      ...
      highPriority = true; # Enable async reprojection with high priority
    ...
    # SteamVR is installed via Steam client - no extra packages needed here

    # ========================================================================
    # VR Performance Configuration
    # ========================================================================
    # NVIDIA-specific: Force GPU-accelerated XWayland for VR overlays
    environment.sessionVariables = mkIf (config.hardware.nvidia.modesetting.enable or false) {
      XWAYLAND_NO_GLAMOR = "0";
    };
  };

  # ==========================================================================
  # Immersed NixOS Module (separate for optional import)
  # ==========================================================================
  flake.modules.nixos.immersed = { pkgs, lib, ... }:
  let
    inherit (lib) mkDefault makeLibraryPath;
  in
  {
    programs.immersed = {
      enable = true;
  ...
  # ==========================================================================
  # Darwin Immersed Module
  # ==========================================================================
  flake.modules.darwin.immersed = { pkgs, ... }:
  let
  ...
  # ==========================================================================
  flake.modules.homeManager.vr = { config, pkgs, lib, osConfig ? {}, ... }:
  let
    inherit (lib) mkIf optionals;

    # Check if WiVRn is enabled at system level
    wivrnEnabled = osConfig.services.wivrn.enable or false;

    # Check if Steam is available (for desktop entries)
    ...
    vr-which-runtime = pkgs.writeShellApplication {
      ...
      text = ''
        ...
        if [ -f "$RUNTIME_32" ]; then
          ...
          echo "  Not configured (32-bit VR games won't work)"
        fi
      '';
    };
    ...
    vr-fix-steamvr = pkgs.writeShellApplication {
      ...
      text = ''
        ...
        if [ "$RUNTIME_FOUND" = false ]; then
          echo "Steam Linux Runtime not found"
  ...
  in
  {
    ...
    home.packages = [
      vr-which-runtime
      vr-fix-steamvr
    ] ++ optionals wivrnEnabled [
      pkgs.wayvr # Desktop overlay for VR
      pkgs.android-tools # ADB for wired VR fallback
    ];

    # OpenXR 64-bit runtime - WiVRn
    # Creates a symlink to WiVRn's OpenXR runtime for Steam and other apps
    ...
    xdg.desktopEntries = mkIf steamEnabled {
      valve-URI-steamvr = {
        name = "URI-steamvr";
