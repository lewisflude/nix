...
let
  ...
  validationTests = import ./validation.nix { inherit lib; };
...
# Combine all tests
lib.recursiveUpdate
  (lib.recursiveUpdate (lib.recursiveUpdate (lib.recursiveUpdate (lib.recursiveUpdate (lib.recursiveUpdate paletteTests modeTests) semanticTests) validationTests) applicationsTests) optionsTests)
  snapshotsTests
