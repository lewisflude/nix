...
with lib; let
  ...
  telemetryData = {
    ...
    packages = {
      ...
      system = "counted_at_build";
      user = "counted_at_build";
    };
    
    # Module statistics
    modules = {
      count = "counted_at_build";
      enabled = []; # Populated at build
    };
    
    # Metadata
    metadata = {
      configPath = config.host.configDir or "/etc/nixos";
      flakeInputs = cfg.trackInputs;
    };
  };
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    ${if pkgs.stdenv.isLinux then "systemd.tmpfiles.rules" else "system.activationScripts.telemetry"} = 
      if pkgs.stdenv.isLinux then [
    ...
    # Automatic collection on rebuild (Linux only - uses systemd)
    ${optionalString (pkgs.stdenv.isLinux && cfg.collectOnBuild) ''
      systemd.services.nix-telemetry-collect = {
