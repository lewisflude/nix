...
pog.pog {
  ...
  version = "1.1.0";
  description = "Advisory: suggest qBittorrent session settings from speed tests and system resources";
  ...
  script =
    helpers: with helpers; ''
            ...
            # Output Functions
            #
            # This tool is advisory only — it deliberately does NOT emit a
            # paste-ready Nix block. The live settings live in
            # modules/qbittorrent.nix under `bittorrentSession`, and several of
            # them (MaxActiveTorrents/MaxActiveUploads = -1, the connection
            # limits) are deliberate private-tracker choices that these generic
            # Azureus-calculator numbers would override. Key names below match
            # the real qBittorrent option names so they can be compared by eye.
            output_json() {
              cat << EOF | jq
      {
        "speed_test": {
        ...
        "BitTorrent.Session": {
          "GlobalMaxUploadSpeed": $UPLOAD_SPEED_LIMIT,
      ...
      ╔══════════════════════════════════════════════════════════╗
      ║   qBittorrent Speed Optimization Calculator v1.1.0       ║
      ...
      Suggested BitTorrent.Session values ($priority Priority):
      ─────────────────────────────────────────────────────────
      GlobalMaxUploadSpeed:       $UPLOAD_SPEED_LIMIT KB/s
      MaxConnections:             $MAX_CONNECTIONS
      MaxConnectionsPerTorrent:   $MAX_CONNECTIONS_PER_TORRENT
      MaxUploads:                 $MAX_UPLOADS
      MaxUploadsPerTorrent:       $MAX_UPLOADS_PER_TORRENT
      ...
      Suggested Preferences.Advanced values:
      ─────────────────────────────────────────────────────────
      PhysicalMemoryLimit:        ''${PHYSICAL_MEMORY_LIMIT} MiB

      Next Steps:
      ─────────────────────────────────────────────────────────
      1. Compare against \`bittorrentSession\` in modules/qbittorrent.nix.
      2. Treat MaxActiveTorrents/MaxActiveUploads as advisory only: this box
         runs private trackers and pins both to -1 (unlimited) on purpose, so
         those two suggestions should normally be ignored.
      3. Edit modules/qbittorrent.nix by hand, then rebuild: nh os switch
      4. Monitor: ./scripts/media/monitor-hdd-storage.sh
      ...
      EOF
            ...
            case "$output_format" in
              ...
              summary)
                ...
                ;;
              *)
                die "Invalid output format: $output_format (must be summary or json)"
                ;;
            esac

            green "✓ Recommendations generated (nothing was changed)"
    '';
}
