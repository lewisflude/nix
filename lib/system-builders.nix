...
let
  ...
  inherit (nixpkgs) lib;
  inherit (inputs) darwin home-manager sops-nix niri ironbar musnix solaar determinate nix-homebrew;
  ...
  mkCommonSpecialArgs = hostConfig: extraArgs: {
  ...
  nixpkgsModule = hostConfig: {
    nixpkgs = {
      overlays = functionsLib.mkOverlays { inherit inputs; inherit (hostConfig) system; };
      config = functionsLib.mkPkgsConfig;
    };
  };

  # Common home-manager modules (Darwin subset)
  darwinHomeModules = lib.optionals (sops-nix != null) [ sops-nix.homeManagerModules.sops ]
  ...
  nixosHomeModules = darwinHomeModules
  ...
  mkHomeManagerModule = hostConfig: sharedModules: [
    ...
    ({ config, ... }: { home-manager.extraSpecialArgs.systemConfig = config; })
  ];
in
{
  mkDarwinSystem = hostName: hostConfig: { homebrew-j178 }:
    (if darwin == null then throw "darwin input is required for mkDarwinSystem" else darwin.lib.darwinSystem) {
      ...
      ++ lib.optionals (home-manager != null) ([ home-manager.darwinModules.home-manager ] ++ mkHomeManagerModule hostConfig darwinHomeModules)
      ++ commonModules;
    };
  ...
  mkNixosSystem = hostName: hostConfig: { self }:
    nixpkgs.lib.nixosSystem {
      ...
      ++ lib.optionals (home-manager != null) ([ home-manager.nixosModules.home-manager ] ++ mkHomeManagerModule hostConfig nixosHomeModules)
      ++ commonModules;
    };
}
