{lib}: let
  ...
  findBool = virtualisation: path:
    let
      value = attrByPath path null virtualisation;
    in
      if isBool value then value else null;

  deriveFlag = virtualisation: globalEnable: pathCandidates:
    let
      result =
        builtins.foldl'
          (acc: path:
...
in {
  mkModulesVirtualisationArgs = {hostVirtualisation ? {}}: let
    ...
    dockerValue =
      deriveFlag virtualisation globalEnable [
        ["enableDocker"]
        ["docker" "enable"]
        ["docker"]
      ];
