...
rec {
  ...
  platformPackages =
    linuxPkgs: darwinPkgs: (lib.optionals isLinux linuxPkgs) ++ (lib.optionals isDarwin darwinPkgs);

  # Architecture-specific package selection
  archPackages =
    x86Pkgs: aarch64Pkgs: (lib.optionals isX86_64 x86Pkgs) ++ (lib.optionals isAarch64 aarch64Pkgs);

  # Conditional module imports
  platformModules =
    linuxModules: darwinModules:
  ...
  platformConfig = linuxConfig: darwinConfig: if isLinux then linuxConfig else darwinConfig;

  # Root group helper (commonly used pattern)
  rootGroup = if isDarwin then "wheel" else "root";

  # Platform-specific package selection
  platformPackage = linuxPkg: darwinPkg: if isLinux then linuxPkg else darwinPkg;

  # Platform-specific state version helper
  platformStateVersion = if isDarwin then 6 else "25.05";
