  ...
  features = {
    ...
    mediaManagement = {
      ...
      qbittorrent = {
        ...
        # SPEED-OPTIMIZED SETTINGS (calculated from 8,216 KB/s upload via Azureus calculator)
        # Generated by: nix run .#calculate-qbittorrent-config -- -u 8216 -p speed
        # Upload speed: 8,216 KB/s (82.16 Mbit/s measured via VPN)
        # Priority: Maximum download/upload speed
        uploadSpeedLimit = 8216; # KB/s - 80% of measured VPN upload (leaves 20% for ACKs)

        # Connection settings
        maxConnections = 600; # Global max connections (optimal for router stability)
        maxConnectionsPerTorrent = 100; # Increased from 80 (+25% peer diversity)

        # Upload slot optimization (massive improvement for concurrent seeding)
        maxUploads = 1643; # Upload slots - increased from 300 (+447% for better seeding)
        maxUploadsPerTorrent = 10; # Per-torrent slots (already optimal)

        # Active torrent limits (speed-optimized for maximum concurrency)
        maxActiveTorrents = 547; # Increased from 150 (+265% concurrent torrents)
        maxActiveUploads = 273; # Increased from 75 (+264% active uploads)
